...
SHIM_TEMPLATE = '''# generated by jev-router's installer - runs {module} from the repository
import runpy, sys
...
def install(providers=ALL, apply=False, uninstall=False):
    ...
    w = Writer(apply)
    if not uninstall:
        w.write(SHIM_HOOK, SHIM_TEMPLATE.format(repo=REPO, module="jev_router.hooks"), "hook shim")
        w.write(SHIM_MCP, SHIM_TEMPLATE.format(repo=REPO, module="jev_router.mcp_server"), "MCP shim")
    steps = []
